from ..io import Config, MazeExporter, ConfigError
...
class MazeGenerator:
    ...
    def transform_to_imperfect_maze(self) -> Generator[None, None, None]:
        ...
        for _ in range(remove_amount):
            ...
            if cell.coord in self.logo_42 or cell.coord == maze.exit:
                continue
            elif neigh in self.logo_42:
                continue
            self.update_wall(maze.get_cell(coord), direction, 0)
            yield
    ...
    def correct_neighbours(self, cell: Cell) -> bool:
        ...
        while free:
            ...
            if self.maze.get_cell(neigh) in self.solution:
    ...
    def generate_42(self) -> None:
        ...
        if self.maze.width > min_width and self.maze.height > min_height:
            ...
            logo.extend(four)
            logo.extend(two)
            forbidden.extend([(center_x + 1, center_y - 1),
                             (center_x, center_y - 1),
                             (center_x + 3, center_y + 1),
                             (center_x + 4, center_y + 1)])
        self.validate_42(forbidden, logo)
        if not self.error_42:
            for coord in logo:
        ...
        try:
            if maze.exit in (forbidden):
                raise ConfigError("'EXIT' position will lead "
                                  "to isolated cells")
            elif maze.exit in logo_42 or maze.entry in logo_42:
