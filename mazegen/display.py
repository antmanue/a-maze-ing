...
class MazeDisplay:
    def __init__(self, grid: list[list[int]]) -> None:
        ...
        self.win_ptr: Any = self.m.mlx_new_window(self.mlx_ptr, 800, 600,
                                                  "A-Maze-ing")
    ...
    def handle_keypress(self, keycode: int, param: int) -> int:
        ...
        if keycode == 65307:
            ...
            print("A desenhar o labirinto...")
            self.draw_maze()
        return 0

    def draw_maze_expose(self, *args: Any) -> None:
        self.draw_maze()
        self.drawn = True
                ...
                if cell_value & 1:
                    for pixel_x in range(x, (x + self.block_size)):
                        self.m.mlx_pixel_put(self.mlx_ptr, self.win_ptr,
                                             pixel_x, y, 0xFFFFFFFF)
                if cell_value & 2:
                    for pixel_y in range(y, (y + self.block_size)):
                        self.m.mlx_pixel_put(self.mlx_ptr,
                                             self.win_ptr, x + self.block_size,
                                             pixel_y, 0xFFFFFFFF)
                if cell_value & 4:
                    for pixel_x in range(x, (x + self.block_size)):
                        self.m.mlx_pixel_put(self.mlx_ptr, self.win_ptr,
                                             pixel_x, y + self.block_size,
                                             0xFFFFFFFF)
                if cell_value & 8:
                    for pixel_y in range(y, (y + self.block_size)):
                        self.m.mlx_pixel_put(self.mlx_ptr, self.win_ptr, x,
                                             pixel_y, 0xFFFFFFFF)